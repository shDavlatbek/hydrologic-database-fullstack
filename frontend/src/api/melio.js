...
export const getNewWellForm = async () => {
  return (await reqApi("/melio/add")).data;
};

export const addNewWell = async (data) => {
  return (await reqApi("/melio/add", data, "POST")).data;
};

export const editWell = async (number, data) => {
  return (await reqApi(`/melio/${number}/edit`, data, "POST")).data;
};

export const getWells = async () => {
  return (await reqApi("/melio/")).data;
};

export const getWellsWithParameters = async (date) => {
  return (await reqApi(`/melio/with_parameters`, {date: date})).data;
};

export const getParameterDates = async () => {
  return (await reqApi(`/melio/parameter/dates`)).data;
};

export const getWell = async (id) => {
  return (await reqApi(`/melio/${id}`)).data;
};
...
export const getParameter = async (well_number) => {
  return (await reqApi(`/melio/parameter`, {well: well_number})).data;
};

export const getPredictions = async (well_number) => {
  return (await reqApi(`/melio/parameter/predict`, {well_number: well_number})).data;
};
...
export const sendExcelData = async (well_number, data) => {
  return (await reqApi(`/melio/${well_number}/parameter/bulk-add`, data, "POST")).data;
};

export const sendConfirmedExcelData = async (well_number, data) => {
  return (await reqApi(`/melio/${well_number}/parameter/bulk-add/confirmed`, data, "POST")).data;
};

export const deleteParameter = async (well_number, date) => {
  return (await reqApi(`/melio/parameter/delete`, {well: well_number, date: date}, "POST")).data;
};

export const calculateParameters = async (well_number, parameter_name, start_date, end_date) => {
  return (await reqApi(`/melio/parameter/calculations`, {well: well_number, parameter_name: parameter_name, start_date: start_date, end_date: end_date}, "GET")).data;
};

export const getHeatmap = async (well_number, parameter_name, start_date, end_date) => {
  const response = await api.get('/melio/parameter/heatmap', {
    params: {
      well: well_number,
